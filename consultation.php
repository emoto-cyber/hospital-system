...
?>
    <?php
require("processconsultation.php");
    ?>
<form class="form" method="post" name="login" action="consultation.php">
                 <h1 class="login-title">Consultation Information </h1>
...
required name="email" 
 ...
 "<?php echo$email?>" >
  ...
  <input type="name" placeholder="Enter diagonostic" name="diag"  class="login-input" required  title=
"Alphabetic and space only max of 30 characters"
value=
 "<?php if (isset($_POST['diag']))
 echo htmlspecialchars($_POST['diag'], ENT_QUOTES);
?>" >
<input type="name" placeholder="Enter treatment" name="treat"  class="login-input" required  title=
"Alphabetic and space only max of 30 characters"
value=
 "<?php if (isset($_POST['treat']))
 echo htmlspecialchars($_POST['treat'], ENT_QUOTES);
?>" >
<input type="name" placeholder="Enter bload pressure" name="blood"  class="login-input" required  title=
"Alphabetic and space only max of 30 characters"
value=
 "<?php if (isset($_POST['blood']))
 echo htmlspecialchars($_POST['blood'], ENT_QUOTES);
?>" >
  <input type="name" placeholder="Enter patient weight" name="weight"  class="login-input" required  title=
"Alphabetic and space only max of 30 characters"
value=
 "<?php if (isset($_POST['weight']))
 echo htmlspecialchars($_POST['weight'], ENT_QUOTES);

?>" >
<input type="name" placeholder="Enter attending doctor name" name="doctor"  class="login-input" required  title=
"Alphabetic and space only max of 30 characters"
value=
 "<?php if (isset($_POST['doctor']))
 echo htmlspecialchars($_POST['doctor'], ENT_QUOTES);
?>" >
