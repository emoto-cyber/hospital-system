...
?>
    <?php
require("processdiseases.php");
    ?>
<form class="form" method="post" name="login" action="diseases.php">
                 <h1 class="login-title">Disease Information </h1>
...
required name="email" 
 ...
 "<?php echo$email?>" >
  ...
  <input type="name" placeholder="Enter disease name" name="disease"  class="login-input" required  title=
"Alphabetic and space only max of 30 characters"
value=
 "<?php if (isset($_POST['disease']))
 echo htmlspecialchars($_POST['disease'], ENT_QUOTES);
?>" >
  <br>
                      <textarea rows="15" cols="38" class="login-input" placeholder="Enter disease description here" name="desc"  pattern="[a-zA-Z][a-zA-Z\s]*" title=
